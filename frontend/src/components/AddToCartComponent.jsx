...
import { deleteBookFromCartApiCall } from '../APIs/bookSaleingApi';
...
<div class="relative overflow-x-auto shadow-md   mx-10  mt-50">
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 ">
        ...
        <tbody>
          {!getCartLoading && getCartSuccessData.Cart.length > 0  ?<>
           {getCartSuccessData.Cart.map((book,i)=>(
             <tr class="border-b border-gray-200 dark:border-gray-700 text-gray-700" key={book._id}>
                <td scope="row" class="px-6 py-4 border-1 ">
<img src={`https://covers.openlibrary.org/b/id/8229224-S.jpg`} alt={book.title} srcset="" />
                </td>
                <td class="px-6 py-4 border-1">
                    {book.title}
                </td>
                <td class="px-6 py-4 border-1  ">
                    {book.price}
