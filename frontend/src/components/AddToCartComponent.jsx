...
import {
  deleteBookFromCartApiCall,
  deleteAllBooksFromCartApiCall,
} from "../APIs/bookSaleingApi";
...
export const AddToCartComponent = () => {
  ...
  const deleteAllBooksFunction = () => {
    deleteAllBooksFromCartApiCall()
      .then((res) => {
        console.log(res);
        if (res.data.message == "All Books Deleted Successfully.")
          fetchCartData();
      })
      .catch((err) => {
        console.log(err);
  ...
  return (
    <>
      {!getCartLoading &&
      getCartSuccessData != undefined &&
      getCartSuccessData.Cart != undefined &&
      getCartSuccessData.Cart.length == 0 ? (
        <div className=" transform translate-y-70  py-4 px-5 text-gray-500">
          <strong className="flex justify-center text-6xl">
            Your cart is currently empty.{" "}
          </strong>
          <small
            className="block pt-2 pl-6 hover:text-blue-500 cursor-pointer"
            onClick={() => {
              navigate("/");
            }}
          >
        ...
        <div class="relative overflow-y-hidden h-auto mx-10  my-50">
              ...
              getCartSuccessData.Cart.length > 0 ? (
                <>
                  {getCartSuccessData.Cart.map((book, i) => (
                    <tr
                      class="border-b border-gray-200 dark:border-gray-700 text-gray-700 "
                      key={book._id}
                    >
                      <td
                        scope="row"
                        class="px-6 py-4 place-items-center border-1 "
                      >
                        <img
                      ...
                      <td class="px-6 py-4 place-items-center border-1">
                        {bookDeletedId == book._id ? (
                          <div role="status">
                            <svg
                              aria-hidden="true"
                              class={`w-8 h-8 text-gray-300 animate-spin fill-green-600`}
                              viewBox="0 0 100 101"
                              fill="none"
          ...
          <div className="py-5 absolute ">
            <button
              className="bg-black text-white text-bold px-5 py-2 mr-5"
              onClick={() => {
                navigate("/");
              }}
            >
            ...
            <button
              className="bg-black text-white text-bold px-5 py-2"
              onClick={() => {
                deleteAllBooksFunction();
              }}
            >
              Clear Cart
            </button>
          ...
          <div className="ml-auto w-fit ">
            ...
            <div className="grid grid-rows-2 gap-2 mb-5">
              <div className="grid grid-cols-2 gap-20 justify-around text-xl">
                <small className="text-right">Sub Total </small>{" "}
            ...
            <button className="bg-yellow-500 text-white text-bold px-5 py-2 absolute right-0">
              Checkout
            </button>
          </div>
        </div>
